# **一、作为行为组件的脚本** 
  ...
  ```
  &emsp;&emsp;为了连接 Unity 的内部架构，脚本将实现一个类，此类从 MonoBehaviour 的内置类派生而来。可以将类视为一种蓝图，用于创建可附加到游戏对象的新组件类型。每次将脚本组件附加到游戏对象时，都会创建该蓝图定义的对象的新实例。<u>*类的名称取自创建文件时提供的名称。**类名和文件名必须相同**才能使脚本组件附加到游戏对象*</u>。
