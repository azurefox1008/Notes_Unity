# **一、作为行为组件的脚本** 
  ...
  ****
  ...
  # **二、变量和函数** 
  Start函数：当该脚本绑定的对象<u>进入场景</u>时**会被调用，无参数，无返回值。
  ```C#
...
**Start**<u>必须启用脚本组件才能被调用</u>，Start在<u>Awake之后、the first update之前</u>调用，即启用脚本组件的条件下可利用Start启动任何所需操作，如初始化代码的任何部分。

Start和Awake in the lifetime of a script attached to an object只能被调用一次，因此不能通过禁用和重新启用脚本来重复执行Start函数。
